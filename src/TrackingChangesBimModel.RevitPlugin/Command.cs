...
public class Command : IExternalCommand
{
    ...
    /// </summary>
    static async void ReportDifferences(
    Document doc,
    ...
    Dictionary<int, string> end_state)
    {
        ...
        foreach (int id in keys)
        {
            if (!start_state.ContainsKey(id))
            {
                ...
                await provider.AddDb(id, userName, ElementDescription(doc, id), "Add");
            }
            else if (!end_state.ContainsKey(id))
            {
                ++nDeleted;
                report.Add(id.ToString() + " deleted");
                await provider.AddDb(id, userName, ElementDescription(doc, id), "Delete");
            }
            ...
            {4
                ...
                report.Add(id.ToString() + " modified "
                  + ElementDescription(doc, id));
                await provider.AddDb(id, userName, ElementDescription(doc, id), "Update");
            }
            else
            {
                ++nIdentical;
